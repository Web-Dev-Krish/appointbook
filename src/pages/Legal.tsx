...
function LegalPage({ type }: { type: 'privacy' | 'terms' }) {
  ...
  useEffect(() => {
    supabase.from('legal_content').select('*').eq('type', type).single()
      .then(({ data }) => {
        setContent(data?.content ?? 'Content coming soon.')
  ...
  return (
    <div className="min-h-screen bg-surface-950">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        {updatedAt && (
          <p className="text-xs text-slate-500 mb-8">Last updated: {new Date(updatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
        )}

        {loading ? (
          <div className="space-y-3">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="shimmer h-3 rounded" style={{ width: `${60 + (i % 4) * 10}%` }} />
            ))}
          </div>
        ) : (
          <div className="prose prose-invert prose-sm max-w-none
            [&_h1]:text-xl [&_h1]:font-bold [&_h1]:text-white [&_h1]:mt-8 [&_h1]:mb-3
